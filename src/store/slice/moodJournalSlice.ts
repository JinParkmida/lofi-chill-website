...
const moodJournalSlice = createSlice({
  ...
  reducers: {
    ...
    updateEntry: (state, action: PayloadAction<IMoodEntry>) => {
      const index = state.entries.findIndex((entry: IMoodEntry) => entry.id === action.payload.id);
      if (index !== -1) {
    ...
    deleteEntry: (state, action: PayloadAction<string>) => {
      state.entries = state.entries.filter((entry: IMoodEntry) => entry.id !== action.payload);
      localStorage.setItem('mood-entries', JSON.stringify(state.entries));
    }
  }
});
